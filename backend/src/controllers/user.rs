...
use crate::{
    dao, models,
    util::{self, CachePool, DbPool, JwtSession, KEY, Result},
};
...
pub fn update_user(
    ...
    // TODO(Matt): This should check to see if the person _can_ edit the user
    if !session.is_valid() {
...
/// User details
pub fn get_user(pool: Data<DbPool>, id: Path<Uuid>) -> Result<HttpResponse> {
...
pub fn login(
    ...
    let (id, token) = util::issue(KEY, user.user_id().to_owned());
